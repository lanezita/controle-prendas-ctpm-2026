  ...
  const ViaRecibo = ({ title }: { title: string }) => (
    <div className="border border-slate-300 p-4 md:p-8 rounded-2xl bg-white mb-8 print:border-none print:shadow-none print:mb-12 print:p-0 relative overflow-hidden">
      {recibo.status === 'cancelado' && (
        <div className="space-y-2 mb-4">
          ...
          <div className="bg-rose-50 border border-rose-200 text-rose-800 p-4 rounded-xl text-xs space-y-1 block print:border-rose-300 print:bg-rose-50/50">
            <p className="font-extrabold uppercase text-[10px] tracking-wider text-rose-700">
      ...
      <div className="text-center mb-8 border-b border-slate-200 pb-6 relative">
        ...
        <h3 className="text-lg md:text-xl font-black text-indigo-700 mt-1">
          Recibo de Entrega de Prendas
        </h3>

        <p className="text-xs md:text-sm font-black text-slate-400 mt-2 uppercase tracking-widest">
          {title}
        </p>

        <p className="text-[10px] md:text-xs font-black text-slate-500 mt-1 uppercase tracking-widest leading-none">
          {SCHOOL_NAME}
        </p>
      </div>
        ...
        <table className="w-full text-left text-xs md:text-sm mb-6 border-collapse min-w-[500px]">
          <thead>
            <tr className="border-b-2 border-slate-800">
              <th className="py-2.5 font-bold uppercase text-[10px] tracking-wider text-slate-900">
                Item/Prenda
              </th>
              <th className="py-2.5 text-center font-bold uppercase text-[10px] tracking-wider text-slate-900">
                Qtd
              </th>
              <th className="py-2.5 text-right font-bold uppercase text-[10px] tracking-wider text-slate-900">
                Pts Base
              </th>
              <th className="py-2.5 text-center font-bold uppercase text-[10px] tracking-wider text-slate-900">
                Mult.
                  ...
                  <td className="py-3">
                    <div className="flex items-center font-semibold">
                      ...
                      {item.campanhaAplicada && (
                        <Zap className="h-3.5 w-3.5 text-amber-500 ml-1.5 shrink-0" />
                      )}
                    </div>
                  </td>
          ...
          <tfoot>
            <tr className="border-t-2 border-slate-800 font-extrabold text-base md:text-lg">
              ...
              <td className="py-4 text-right text-indigo-700">
